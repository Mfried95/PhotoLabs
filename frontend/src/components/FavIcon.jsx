...
export function FavIcon({
  ...
  // State to track whether the icon is clicked or not
  const [isClicked, setIsClicked] = useState(false);

  const handleClick = () => {
    // Toggle the clicked state when the icon is clicked
    setIsClicked(!isClicked);

    // Update the likes count based on the clicked state
    if (isClicked) {
  ...
  return (
    ...
    >
      <path
        // SVG path defining the shape of the favorite icon
        ...
        fill={isClicked || likes ? "red" : "white"}
        stroke={stroke}
